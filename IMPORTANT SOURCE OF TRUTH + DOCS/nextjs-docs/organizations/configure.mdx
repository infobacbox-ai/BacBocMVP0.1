...
To disable organizations, you can set the `organizations.enabled` flag to `false` in the `config/index.ts`.
...
To require an organization for a user to be able to access the application, you can set the `organizations.requireOrganization` flag to `true` in the `config/index.ts`.
...
You can hide the organization selection in the UI by setting the `organizations.hideOrganization` flag to `true` in the `config/index.ts`.
