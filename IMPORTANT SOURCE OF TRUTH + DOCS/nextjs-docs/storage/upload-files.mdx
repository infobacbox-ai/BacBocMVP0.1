...
As explained on the [overview page](/docs/nextjs/storage/overview), supastarter uses the presigned URLs to upload files to your storage provider.
So what we need to do first is to extend the `api/signed-upload-url` API route to be able to upload files to the `documents` bucket.

```ts title="packages/api/src/routes/uploads.ts"
