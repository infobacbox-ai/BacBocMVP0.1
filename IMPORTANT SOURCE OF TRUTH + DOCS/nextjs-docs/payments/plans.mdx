...
```tsx title="/apps/web/modules/saas/payments/hooks/plan-data.ts"
...
_Note: When you remove one of the existing plans, make sure to remove the corresponding entry in the `/apps/web/modules/saas/payments/hooks/plan-data.ts` file, otherwise you will get a type error._