...
description: Learn how use permissions and access control in your supastarter frontend application.
---

We have already guided you through the process of [how implement access control in the API routes](/docs/nextjs/api/protect-endpoints) of your application.
In this guide we will show you how you can protect pages and display UI based on the users role or permissions.
...
_Note: When you are inside the `/apps/web/app/(saas)/app` directory, you don't need to check if the user is authenticated, because the session is verified in the middleware._
...
__Note: You always want to check the permission on the server side first to avoid any security issues.__
